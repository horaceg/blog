...
In this section, and in this section only, we assume that $\pi$ is additive (see definition below).
...
It is another way to say that $\pi$ is __additive__.
...
Which means that $\pi$ is a measure, which is finite. 
...
See the [appendix](#appendix) for the construction of surebets with the help of reversing (negative) bets.

## Dutch book

Provided that we can back or lay any bet offered to us, that is, for a given odds, play the role of the punter _or_ the role of the bookie. In the case of incoherent odds, a bettor can build a _dutch book_ against the bookie, that is a set of bets that constitutes a surebet.
...
If we assume that $\pi$ is additive, it is immediate that we have $\hat{p}_{\Omega} = 1$ and thus $\hat{p}$ is a probability measure.
...
### Single event - Convexity

Observe that for any given event, $\{F_A^{\pi}\ | \pi \ge 0 \}$ is a convex set:

For $\alpha \in [0, 1]$, we have:

$\alpha F_A^{\pi_1} + (1 - \alpha) F_A^{\pi_2} = F_A^{\alpha \pi_1 + (1 - \alpha)\pi_2}$.
...
\begin{equation*}
\begin{split}
x F^{\pi_1}_A + y F_A^{\pi_2} & = (x + y) (\frac{x}{x + y}F_A^{\pi_1} + \frac{y}{x+y}F_A^{\pi_2}) \\ 
& = (x + y) F_A^{\alpha \pi_1 + (1 - \alpha) \pi_2}
\end{split}
\end{equation*}
$$
...
### Single event and its complimentary
...
\begin{equation*}
\begin{split}
(1 - \alpha) F_A^{\pi} + \alpha F_{\overline{A}}^{\pi} & = (1 - 2 \alpha) F_A^{\pi} - \alpha \epsilon_A \\
& = (2 \alpha - 1) F_{\overline{A}}^{\pi} - (1 - \alpha) \epsilon_A
\end{split}
\end{equation*}
$$

where $\epsilon_A = \pi_A + \pi_{\overline{A}} - 1$

In the _smart bookie_ setting, $\epsilon_A > 0$.

for arbitraty amounts $x, y \geq 0$, we have:

$$
\begin{equation*}
\begin{split}
x F_A^{\pi} + y F_{\overline{A}}^{\pi} & = (x - y)F_A^{\pi} - y \epsilon_A \\
& = (y - x)F_{\overline{A}}^{\pi} - x \epsilon_A 
\end{split}
\end{equation*}
$$
...
We arrive thus at the combining formula:

$$
\begin{equation*}
\begin{split}
x F_A^{\pi_1} + y F_A^{\pi_2} + z F_{\overline{A}}^{\pi_3} & = (x + y - z) F_A^{\alpha \pi_1 + (1 - \alpha) \pi_2} - z [\alpha \pi_1 + (1 - \alpha) \pi_2 + \pi_3 - 1]
\end{split}
\end{equation*}
$$
